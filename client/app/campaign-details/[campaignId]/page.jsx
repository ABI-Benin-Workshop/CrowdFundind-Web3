...
import CountBox from '@/components/count-box.jsx';
...
export default function CampaignDetails({ params }) {
  ...
  const { address, getCampaign, getDonations, donate } = useContext(StateContext);
  const fetchCampaigns = async () => {
    setIsLoading(true);
    const filteredCampaign = await getCampaign(params.campaignId);
    setCampaign(filteredCampaign);
    setRemainingDays(daysLeft(filteredCampaign.deadline))
    const res = await getDonations(filteredCampaign.pId);
    setDonators(res);
    setIsLoading(false);
  }
  useEffect(() => {
